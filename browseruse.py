...
async def get_cv():
    # Implementation of get_cv function
    pass
...
async def main():
    ...
    if not CV.exists():
        logging.error("CV does not exist")
        print(f"CV file not found at {CV}")
        cv_path = input("Enter path to CV file: ").strip()
        CV = Path(cv_path)
        if not CV.exists():
            print("Invalid CV path. Exiting...")
            return

    cv_result = read_cv()
    if cv_result.error:
        logging.error(f"CV reading error: {cv_result.error}")
        return
    logging.info("CV read successfully")

    # Ensure API keys are available
    config = load_api_keys()
    required_keys = ['OPENAI_API_KEY'] if 'OPENAI_API_KEY' in config else ['AZURE_OPENAI_KEY', 'AZURE_OPENAI_ENDPOINT']
    missing_keys = [key for key in required_keys if key not in config]
    
    if missing_keys:
        print(f"Missing required API keys: {', '.join(missing_keys)}")
        print("Please run with API key configuration first.")
        openai_key = input("Enter OpenAI API key (leave blank to skip): ").strip() or None
        azure_key = input("Enter Azure OpenAI key (leave blank to skip): ").strip() or None
        azure_endpoint = input("Enter Azure OpenAI endpoint (leave blank to skip): ").strip() or None
        huggingface_key = input("Enter Hugging Face API key (leave blank to skip): ").strip() or None
        gemini_key = input("Enter Gemini API key (leave blank to skip): ").strip() or None
        
        configure_api_keys(openai_key, azure_key, azure_endpoint, huggingface_key, gemini_key)
        config = load_api_keys()
        ...
        missing_keys = [key for key in required_keys if key not in config]
        if missing_keys:
            print(f"Still missing required API keys: {', '.join(missing_keys)}")
            print("Exiting...")
            return
    ...
    print("2. Azure OpenAI (requires API key and endpoint)")
    print("3. Hugging Face (requires API key)")
    print("4. Gemini (requires API key)")
    print("5. Use any available API (will try in the order above)")
    
    model_choice = input("Select option (1-5): ").strip()
    model_provider = {
        "1": "openai",
        "2": "azure",
        "3": "huggingface",
        "4": "gemini",
        "5": "any"
    }.get(model_choice, "any")

    # Setup browser
    browser = initialize_browser()
    
    # Setup LLM
    model = await setup_llm_model(model_provider)
    
    # Improved company selection
    print("\nSelect companies to search:")
    print("1. Choose from list")
    print("2. Enter company names manually")
    print("3. Use top companies")
    print("4. Use all companies")
    
    company_choice = input("Select option (1-4): ").strip()
    
    if company_choice == "1":
        print("Select companies to search (comma-separated numbers, or 'all' for all):")
        for i, company in enumerate(TOP_TECH_COMPANIES):
            print(f"{i+1}. {company}")
        
        selection = input("Enter selection: ").strip().lower()
        
        if selection == 'all':
            selected_companies = TOP_TECH_COMPANIES
        else:
            try:
                indices = [int(idx.strip()) - 1 for idx in selection.split(',')]
                selected_companies = [TOP_TECH_COMPANIES[idx] for idx in indices if 0 <= idx < len(TOP_TECH_COMPANIES)]
            except:
                print("Invalid selection. Using top 5 companies.")
                selected_companies = TOP_TECH_COMPANIES[:5]
    
    elif company_choice == "2":
        company_names = input("Enter company names (comma-separated): ").strip()
        selected_companies = [name.strip() for name in company_names.split(',') if name.strip()]
    
    elif company_choice == "3":
        selected_companies = TOP_TECH_COMPANIES[:5]
    
    else:  # Option 4 or invalid choice
        selected_companies = TOP_TECH_COMPANIES
    
    # Ask for job type preferences
    job_type = input("Enter job type (e.g., ML, Software Engineer): ").strip() or "ML"
    position_type = input("Enter position type (e.g., internship, full-time): ").strip() or "internship"
    
    print(f"Searching for {job_type} {position_type} positions at: {', '.join(selected_companies)}")
    
    # Generate tasks for each company
    tasks = []
    for company in selected_companies:
        task_result = generate_job_search_task(company, job_type, position_type)
